<template>
  <canvas id="star-point" ref="starPoint"></canvas>
</template>
...
export default {
  ...
  methods: {
    ...
    setCanvasSize() {
      this.c_size.width = this.sp_c.width =
        window.innerWidth ||
        document.documentElement.clientWidth ||
        document.body.clientWidth;
      this.c_size.height = this.sp_c.height =
        window.innerHeight ||
      ...
      this.random_points.forEach(function(r, idx) {
        ...
        for (i = idx + 1; i < _this.all_array.length; i++) {
          ...
          if (null !== e.x && null !== e.y) {
            ...
            dist < e.max &&
              (e === _this.current_point &&
                dist >= e.max / 2 &&
                ((r.x -= _this.mouseV * x_dist),
                (r.y -= _this.mouseV * y_dist)), //靠近的时候加速
              (d = (e.max - dist) / e.max),
              ctx.beginPath(),
              (ctx.lineWidth = d / 2 + _this.line_width),
              (ctx.strokeStyle =
                "rgba(" + _this.hex2rgb(_this.color) + "," + (d + 0.2) + ")"),
              ctx.moveTo(r.x, r.y),
              ctx.lineTo(e.x, e.y),
              ctx.stroke());
          }
